...
const createCanvas = async ({
	...
	if (Number.isNaN(Number.parseInt(extraWidthValue))) {
	...
	return new Promise((resolve, reject) => {
		...
		img.onload = () => {
			...
			// eslint-disable-next-line radix
			const EXTRA_WIDTH_VALUE = Number.parseInt(extraWidthValue);
			const result = [];
			...
			groupsWords.forEach((words, i) => {
				...
				words.forEach((w, j) => {
					...
					canvas.setAttribute('draggable', draggable);
					...
					ctx.drawImage(img, startXPointCropImage, startYPointCropImage,
						canvasWidth + radius, canvasHeight, 0, 0, canvasWidth + radius, canvasHeight);
